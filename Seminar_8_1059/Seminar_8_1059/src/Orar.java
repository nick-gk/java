...
//        System.out.println(componentaSerii);
        ...
        String seria = null;
        for (Map.Entry<String, List<String>> cheie : componentaSerii.entrySet()) {
            List<String> grupeSerie = cheie.getValue();
            for (String g : grupeSerie) {
                if (grupeSerie.contains(grupa)) {
                    seria = cheie.getKey();
                    break;
                }
            }
        ...
        programari.stream()
//                .sorted((a, b) -> a.getZiua().toString().compareTo(b.getZiua().toString()))
//                .collect(Collectors.groupingBy(Programare::getFormatie))
//                .collect(Collectors.filtering((f) -> programari.stream().filter(f -> f.getFormatie().equals(grupa) ||
//                f.getFormatie().equals(finalSeria))))
                .filter(f -> f.getFormatie().equals(grupa) || f.getFormatie().equals(finalSeria))
//                .forEach((formatie, orarFormatie) -> orarFormatie.stream().filter(f -> f.getFormatie().equals(grupa) ||
//                        f.getFormatie().equals(finalSeria))
                .forEach(g -> {
                            if (!g.getZiua().equals(ziNoua.toString())) {
                                System.out.printf("%-10s ",
                                        g.getZiua());
                                ziNoua.set(g.getZiua());
                            } else {
                                System.out.printf("           ");
                            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (var fisier = new BufferedReader(new FileReader("dataIN\\profesori.txt"))) {
        ...
        try (var fisier = new BufferedReader(new FileReader("dataIN\\programari.txt"))) {
...
//        System.out.println(componentaSerii);
        afisareOrarGrupa("1046", programari, componentaSerii);
    }
}
